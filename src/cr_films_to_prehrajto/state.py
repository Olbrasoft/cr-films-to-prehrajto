...
class StateStore:
    ...
    def uploaded(self, cr_film_id: int) -> bool:
        return bool(self.film(cr_film_id).get("upload", {}).get("target_video_id"))

    def burned_source_ids(self, cr_film_id: int) -> set[str]:
        return {
            str(a["source_id"])
            for a in self.film(cr_film_id).get("attempts", [])
    ...
    def pending_partial_upload(self, cr_film_id: int) -> dict | None:
        if self.uploaded(cr_film_id):
            return None
        for attempt in reversed(self.film(cr_film_id).get("attempts", [])):
