...
states in `Olbrasoft/prehrajto-to-prehrajto`, reconciled against a complete
one-time snapshot of both active and deleted target-account pages, and is
updated from `state/pilot.json` after every upload workflow. Deleted and absent
...
```bash
cr-films-index reconcile \
  --snapshot data/catalog.json \
  --index state/account-index.json \
  --inventory data/live-account-inventory.json \
  --deleted-inventory data/deleted-account-inventory.json \
...
Only a live historical video ID or an exact normalized Czech/original title
with a compatible year suppresses a film. Related titles and deleted videos
remain in the missing backlog.
